...
file "#{consul_template_config_path}/telegraf_rabbitmq_inputs.hcl" do
  ...
  content <<~HCL
    ...
    template {
      ...
      # return within 30s (configurable), and it must have a successful exit code.
      # Consul Template is not a replacement for a process monitor or init system.
      command = "chown #{node['telegraf']['service_user']}:#{node['telegraf']['service_group']} #{telegraf_config_directory}/inputs_rabbitmq.conf && systemctl restart #{telegraf_service}"
